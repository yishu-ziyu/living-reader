...
- 2026-08-09: Reaching for a local translation model when the current Sol model can translate added a dependency and produced worse Chinese → use the current Agent model for book translation, keep per-block provenance, and reject low-quality batches before expanding.

- 2026-08-10: A fixed four-station recipe with canned actions is not a real-time Agent world → prebuild only reviewed primitives and safety rules; the Agent must compose, explain, and extend the spatial world from the current source, reader intent, and live events.
- 2026-08-10: Clicking a place list to relabel “where I am” is not a walkable world → the reader or avatar must move through space; place panels are secondary, never the primary spatial interaction.
- 2026-08-10: Overlapping 1-bit sprites, buildings, and labels that hide each other fail visual acceptance immediately → enforce occupancy, depth sort, and one clear focal subject before any art polish or focus-panel work.
- 2026-08-10: Building a novel walkable/embeddable world renderer from memory wastes the chance to reuse battle-tested open source → research embeddable candidates and product insertion constraints before coding the next spatial prototype.