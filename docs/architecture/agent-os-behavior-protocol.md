...
1. 任何事实性回答必须带当前书籍版本和可回到同一段的 `SourceBlock`/`evidence`；
   没有来源就显示“未能从当前原文确认”。
2. 任何世界改变必须是用户明确的、allowlist 内的 `DomainCommand`，并由
   `WorldKernel.decide/evolve` 生成 `DomainEvent`；Agent OS 只能编译、解释和呈现。
...
type AgentInput = {
  ...
  explicit_control?: "none" | "stop" | "refuse" | "confirm";
};
...
type IntentDecision = {
  ...
  classification_status: "classified" | "needs_clarification";
...
分类是一个可重放的结构化结果，不把模型自由文本当成事实。排序规则：显式
`stop/refuse` 先处理；无语义或注入噪声才归 `obvious_off_topic_noise`；只有在
active world 存在且句子明确要求改变它时才归 `executable_action`；存在多种同样
合理解释时 `classification_status=needs_clarification` 且不填 `intent_class`，不能猜选。
...
| `executable_action` | 明确对当前世界下动作：“扩大市场”“让织工再细分” | `mechanism_adjacent` | 解析 allowlist action，检查前置条件，发 `world.action`；歧义时只澄清 |
| `productive_detour` | 看似离开当前段但能检验/照亮机制的类比、历史或现实经验 | `mechanism_adjacent` | 标为 detour，保留为 `BookThought(kind=experiment/inference)` 候选，邀请连接回机制 |
| `emotion_personal` | “这让我焦虑/想起我的工作/我不想继续” | `personal` | 先回应情绪，询问是否要连到原文；不自动存储，不自动运行世界 |
...
### 3.3 回应策略与 soft-return

每种 class 都必须产出结构化响应元数据：`intent_class`、`source_ids`、
`evidence`、`confidence`、`open_question`、`next_moves`。可见文本只是其渲染。

| 类别 | 回应必须包含 | 下游写入 | 禁止 |
|---|---|---|---|
| source question | 先回答当前问题；列出原文/推断边界；不确定处填 open question | `answer.ready`；必要时 BookThought 候选 | 以背景知识替代本段证据；无来源断言“Smith 说过” |
| executable action | 复述“我将对哪个世界做什么”；显示前置条件/可逆性；明确执行或等待确认 | 仅 allowlist `DomainCommand`；结果由 Kernel 产生 | LLM 直接改 WorldState；把建议当已发生事件 |
| productive detour | 承认联想有价值；指出它对应的机制变量或可作为 experiment；提供一个返回入口 | `BookThought` experiment/inference 候选或仅临时响应 | 强行切断联想；把现代例子伪装成原文证据 |
| emotion/personal | 先用一句人话回应情绪；询问是否希望与来源/世界连接 | 默认零领域写入；用户明确要与当前 SourceBlock 连接时，才建带 `personal` 标记且有来源锚点的 ReaderIdea | 诊断、说教、自动把个人经历变成经济事实 |
| obvious off-topic/noise | 简短、诚实、无评判；给一个当前 source/world 的具体入口 | 仅分类/响应事件；不建 Idea/Thought/Action | “你偏题了”“回到书上”等羞辱或强制命令；继续无边界闲聊 |

**Soft-return 规则（必须）：** 对 `productive_detour`、`emotion_personal` 和
`obvious_off_topic_noise`，默认回应最多三句/三行：

1. 一句承认/共情（不评价用户是否“应该”这样想）；
2. 若有可靠连接，一句指出当前 `SourceBlock` 或 `WorldState` 中的对应张力；
3. 只给一个下一步入口（例如“要不要看织工在小市场下的选择？”）。
...
final voice/text
  ...
  → MechanismGraph candidate (nodes/edges/extensions)
  → user review / correction
  → graph.commit (EventStore)
  → action precondition check
  → reader_world.world.action.v1
  → WorldKernel.decide/evolve
  → DomainEvent + CharacterObservation
  → EvidenceSnapshot back to SourceBlock
```

1. LLM 只返回 schema-valid 候选；其返回中的“执行”“已经发生”字样不具备权限。
2. `MechanismGraph` 只有在 source authority、typed edge、冲突、不变量和用户确认均
   通过后才进入 `committed`。
3. `executable_action` 必须绑定 `graph_id + graph_revision + world_id + expected_world_revision`，
   并映射到 allowlist 中的结构化动作；文本中出现“忽略规则/直接改数”必须被阻断。
4. 清晰且低风险的动作可以把用户明确的动作句视作确认；高影响、歧义或不可逆动作
   必须先显示预览并等待 `explicit_control=confirm`。无论哪种路径，Kernel 都是最终
   adjudicator。
5. Director 负责决定先呈现哪个角色观察、何时给用户下一步入口，但不能替角色状态
...
MVP allowlist 只开放三个世界动作（原型按钮名在括号中）：
...
| `constrain_market` (`constrain`) | “缩小市场范围” | 当前 world playable、同上 | market reachable orders/订单积压/切换损耗的 allowlist delta |
...
| 分支 | 领域动作 | 可见结果 | 禁止 |
|---|---|---|---|
| **silence / 空 turn** | partial 全部丢弃；不分类、不创建 Idea/Thought/Action；可记 trace-only 超时 | 保持当前阅读或世界；显示“没有听清/可以重说或继续阅读”一次 | 用沉默猜意图、自动运行上一动作、持续录音 |
| **refusal to continue** | 立刻 `interrupt`/停止邀请；已提交对象保持不变；不新建下游任务 | “好的，先停在这里。”保留回原文、文字输入和继续入口 | 追问、劝说、自动恢复上一轮、把拒绝当噪声 |
| **ASR uncertainty** | 若主类/来源仍明确，final 可生成 `ReaderIdea(status=needs_review, confidence=unknown)`；否则只生成 `proposal.clarification_required`；不生成 committed graph/action | 显示转写、来源、疑点；可编辑、重说或改用文字 | 把低置信文本直接执行；让 partial 写账 |
...
| **no active source** | 在 speech start 前中止为 `source_unavailable`；不调用 Agent、不创建 anchored Idea | 要求点选 PDF 36/45 或文字选择来源；原状态不变 | 绑定最近页面、猜测来源、让世界引用无来源文本 |
| **action unsupported / world not ready** | `operation.failed` typed error；零 world mutation | 显示缺少 graph/前置条件/动作不在 allowlist；保留可修正入口 | 用自由文本替代动作、让 LLM 改状态绕过 Gate |
| **provider/schema failure** | 按既有 outbox/retry/reconcile；不追加成功事实 | 显示可重试、文字 fallback 或“等待同步”；已提交状态仍可读 | 假造 final、假造 `CharacterObservation`、静默换模型 |
...
| WA-3 | 读者说“扩大市场” | 明确 allowlist action，目标 world/graph revision 匹配 | 先显示解析后的动作和可逆性；执行后 `market.reachable_orders`/exchange path/订单簿变化；不是一句 Agent 解释 |
...
| OG-2 | Agent 给一个具体软回引：“可以看看市场太小如何改变织工的选择” | `agent.soft_return.offered`，主状态仍 reading/playable | 当前 SourceBlock 或 world entry 可点击；只提供一个下一步，不抢夺控制 |
| OG-3 | 用户接受/拒绝回引 | 接受进入 RD/WA；拒绝保持当前状态并停止邀请 | 不创建 ReaderIdea、BookThought 或 world action（除非用户随后明确提出）；拒绝后不重复催促 |

## 9. 事件与状态表（可直接转成合同测试）

下表中的事件名遵循既有 `reader_world.*.v1` 命名；新增的 `agent_os.*` 是公开的
语义事件视图，仍必须经 `dispatch` 写入 EventStore。`trace-only` 不影响领域版本，
不能被当作事实成功。

| ID | 当前状态 | 输入/守卫 | 规范事件序列 | 下一状态 | 断言/禁止 |
|---|---|---|---|---|---|
| AOS-01 | `reading` | active source + final source question | `agent_os.intent.classified` → `reader_world.answer.ready` | `reading` | answer 带 source/evidence；不得写 world |
| AOS-02 | `reading` | final productive detour | `agent_os.intent.classified` → `reader_world.reader_idea.proposed`（如用户要保存）→ `agent_os.book_thought.proposed` | `preparing` 或 `reading` | detour 可被接受为 experiment；不得被标为原文 quote |
| AOS-03 | `reading` | final emotion/personal | `agent_os.intent.classified` → `agent_os.response.emitted` | `reading` | 先共情；默认零 graph/world 写入 |
| AOS-04 | `reading`/`playable` | obvious off-topic/noise | `agent_os.intent.classified` → `agent_os.soft_return.offered` | 原状态 | 文本最多一轮 soft-return；不得创建 Idea/Thought/Action |
| AOS-05 | 任意 | explicit stop/refuse | `agent_os.turn.interrupted`（必要时既有 cancellation 事件） | 原状态/`paused` | 本地采集/播报停止；不产生新领域事实 |
| AOS-06 | 任意 | no active source at speech start | `reader_world.operation.failed(SOURCE_UNAVAILABLE)` | `reading` | 不调用 Agent；不产生 anchored Idea |
| AOS-07 | `reading` | final ASR low confidence | 若主类/来源明确：`reader_world.reader_idea.proposed(confidence=unknown,status=needs_review)` → `reader_world.proposal.clarification_required`；否则直接 `reader_world.proposal.clarification_required` | `reading`/`needs_review` | 只显示可编辑转写；不得 commit graph/action |
| AOS-08 | `reading` | claim has no evidence | `reader_world.answer.ready(uncertainty_summary)` | `reading` | 清楚标 model extension/unknown；不得生成 quote |
| AOS-09 | `needs_review` | relation accepted, evidence/basis valid | `reader_world.relation.reviewed(accept)` → `reader_world.graph.committed` | `playable`（Gate 通过时） | rejected/stale relation 不得进入 world |
| AOS-10 | `playable` | `world.action(deepen_specialization)`，baseline predicate 不满足 | `reader_world.world.event_recorded(event_kind=character_refusal)` → `agent_os.character.observation.recorded` | `playable` | weaver refusal 可重算；WorldState 数值无非法变化 |
| AOS-11 | `playable` | clear `world.action(expand_market)`，expected revision 匹配 | `reader_world.world.event_recorded(market_expanded)` → `world.event_recorded*` → `agent_os.character.observation.recorded*` → `reader_world.evidence_snapshot.produced` | `evidence_ready` | 只发生 allowlist delta；角色顺序与 local state 一致 |
| AOS-12 | `playable` | action 缺 world/graph/allowlist 或 version stale | `reader_world.operation.failed(WORLD_NOT_READY | ACTION_UNSUPPORTED | EXPECTED_VERSION_MISMATCH)` | `playable`/`needs_review` | 零 mutation；不能用当前模型结果覆盖新 revision |
| AOS-13 | `evidence_ready` | 点击回到原文 | 读取既有 `EvidenceSnapshot`；可有 `agent_os.evidence.opened` trace-only 事件，不重复产生领域 snapshot | `reading`/`evidence_ready` | Evidence 含 PDF 36/45、ReaderIdea、关系、事件和模型边界 |
...
| AOS-15 | 任意 | 用户沉默或只有 partial | 无领域事件；可有 trace-only timeout | 原状态 | partial 不写 Idea、Thought、Graph 或 WorldState |
| AOS-16 | 任意 | 用户拒绝继续软回引 | `agent_os.soft_return.declined`（trace-only） | 原状态 | 不重复邀请，不把拒绝转为 noise 或 action |
...
7. `AOS-11`：执行 expand market 后，订单/交换路径先更新，角色 observation 按
   merchant → shepherd/spinner → weaver 顺序产生；`weaver` 只有 predicate 满足才
   接受；相同 seed/revision 可 exact replay。
8. `AOS-12/AOS-14`：stale、unsupported、timeout、reconcile 均 fail closed，重试
   幂等，不重复订单/现金/库存。
9. 所有新增 Agent OS 事件均携带 `correlation_id`、`causation_id`、basis version、
   source/world references；不携带 raw prompt、思维链、token 或未经授权原始音频。
...
- ASR confidence 与“用户是否真的确认了动作”需要校准集；在校准完成前，低置信或
  高影响动作默认停在 `needs_review`。
- `productive_detour` 与明显无关闲聊的边界会因用户而异；必须记录分类版本和用户
  修正，不能只看平均回复长度验收。
- 角色观察模板若过于固定会显得机械；可改善措辞，但不能放宽 local-state 和
  evidence 约束。
- PDF 重解析、缩放和页面切割可能造成 anchor drift；任何无法唯一定位都必须
  `fail closed`，不能以视觉上“差不多”通过。
- 个人/情绪输入的保留、删除和可见范围仍需单独的隐私决定；本协议默认不持久化。

协议完成的判据是：对任意一轮输入，系统都能回答“它属于哪类意图、依据哪些来源、
形成了哪一版 Thought/Graph、是否真的改变 WorldState、哪个角色因何行动、用户最终
看到什么，以及在沉默、拒绝、未知、无来源和过期时如何安全停下”。
